...
class Interface:
    ...
    def run(self, ):
        ...
        self.current_joints_pose = np.asarray(self.controller.robot_model.forward_tranformation(self.x_start)).flatten()
    ...
    def checkFinishManipulator(self):
        ...
        '''
        if (ca.norm_2(self.current_joints_pose[:3] - self.x_target) <= 0.01):
            self.task_flag = 'finish'

    def calcLocalRefTraj(self):
        '''
        TODO: 
        compute global reference trajectory 
    ...
    def calcLocalRefPose(self):
        self.local_traj_ref = np.tile(self.x_target, (self.controller.N + 1, 1))
