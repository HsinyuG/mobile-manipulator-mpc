...
class Interface:
    def __init__(self, dt, t_move, t_manipulate, base_x_start, joint_x_start, global_pose_target, controller_base, controller_manipulator, physical_sim=False):
        ...
        self.global_pose_target = global_pose_target
        self.working_radius = 0.6
        # calculate base target by hard coded logic, should by sampling and checking reachability
        self.base_x_target = np.array([
        ...
        self.x_start = np.hstack((base_x_start, joint_x_start))
        self.base_x_start = base_x_start
    ...
    def timerCallback(self):
        ...
        if self.task_flag == 'manipulate':
            self.joint_u_log.append(copy.deepcopy(np.asarray(self.command)))
        else:
    ...
    def globalPlan2D(self):
        ...
        self.traj_ref = np.array([
            np.linspace(self.base_x_start[0], self.base_x_target[0], int(traj_length + 1)),
            np.linspace(self.base_x_start[1], self.base_x_target[1], int(traj_length + 1)),
            np.linspace(self.base_x_start[2], self.base_x_target[2], int(traj_length + 1)),
            np.zeros(int(traj_length + 1)),
            np.zeros(int(traj_length + 1)),
            np.zeros(int(traj_length + 1))
        ]).T
    ...
    def globalPlanManipulator(self):
        ...
        joint_x_target = self.controller.robot_model.inverse_transformation(self.joint_x_start, self.pose_target)
        self.traj_ref = np.array([
            np.linspace(self.joint_x_start[0], joint_x_target[0], int(traj_length + 1)),
            np.linspace(self.joint_x_start[1], joint_x_target[1], int(traj_length + 1)),
            np.linspace(self.joint_x_start[2], joint_x_target[2], int(traj_length + 1)),
        ]).T
    ...
    def checkFinishManipulator(self):
        ...
        except: print('first loop of manipulate, check not performed')
    ...
    def plotManipulator(self, is_mobile=False):
        self.joint_x_log = np.asarray(self.joint_x_log)
        self.joint_u_log = np.asarray(self.joint_u_log)
        self.manipulator_pose_log = np.asanyarray(self.manipulator_pose_log)
        # Plot the results
        t = np.arange(len(self.joint_x_log))

        idx_offset = 6 if is_mobile else 0
        # q1 q2 q3
        plt.figure()
        plt.subplot(121)
        plt.plot(t, self.joint_x_log[:, 0+idx_offset], label='q1')
        plt.plot(t, self.joint_x_log[:, 1+idx_offset], label='q2')
        plt.plot(t, self.joint_x_log[:, 2+idx_offset], label='q3')
        plt.xlabel('Time Step')
        ...
        plt.subplot(122)
        plt.plot(t[:-1], self.joint_u_log[:, 0+idx_offset], label='dq1')
        plt.plot(t[:-1], self.joint_u_log[:, 1+idx_offset], label='dq2')
        plt.plot(t[:-1], self.joint_u_log[:, 2+idx_offset], label='dq3')
        plt.xlabel('Time Step')
    ...
    def plot2D(self):
        self.base_x_log = np.asarray(self.base_x_log)
        self.base_u_log = np.asarray(self.base_u_log)
        # Plot the results
        t = np.arange(len(self.base_x_log))

        # x y psi
        plt.figure()
        plt.subplot(311)
        plt.plot(t, self.base_x_log[:, 0])
        plt.xlabel('Time Step')
        plt.ylabel('x')
        plt.grid()

        plt.subplot(312)
        plt.plot(t, self.base_x_log[:, 1])
        plt.xlabel('Time Step')
        plt.ylabel('y')
        plt.grid()

        plt.subplot(313)
        plt.plot(t, self.base_x_log[:, 2])
        plt.xlabel('Time Step')
        plt.ylabel('psi')
        plt.grid()
        ...
        plt.figure()
        plt.subplot(311)
        plt.plot(t, self.base_x_log[:, 3])
        plt.xlabel('Time Step')
        plt.ylabel('dx')
        plt.grid()

        plt.subplot(312)
        plt.plot(t, self.base_x_log[:, 4])
        plt.xlabel('Time Step')
        plt.ylabel('dy')
        plt.grid()

        plt.subplot(313)
        plt.plot(t, self.base_x_log[:, 5])
        plt.xlabel('Time Step')
        plt.ylabel('dpsi')
        plt.grid()
        ...
        plt.figure()
        plt.subplot(211)
        plt.plot(t[:-1], self.base_u_log[:, 0])
        plt.xlabel('Time Step')
        plt.ylabel('v_dot')
        plt.grid()

        plt.subplot(212)
        plt.plot(t[:-1], self.base_u_log[:, 1])
        plt.xlabel('Time Step')
        plt.ylabel('w_dot')
        plt.grid()
